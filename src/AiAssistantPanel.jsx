...
export default function AiAssistantPanel({ sections, projects, stages, employees, currentUser, onCreateTasks }) {
  ...
  const meetingQueueRef = useRef([]);
  const meetingWorkersRef = useRef(0);
  ...
  async function transcribePreparedSegment(jobId, index, maxAttempts = TRANSCRIBE_MAX_ATTEMPTS) {
    let lastError;
    for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
      try {
        return await apiJson('/api/ai/transcribe-prepared-segment', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ jobId, index, language: 'ru' }),
        });
      } catch (error) {
        lastError = error;
        if (attempt >= maxAttempts || !isRetryableTranscriptionError(error)) throw error;
    ...
    throw lastError || new Error('Не удалось расшифровать часть записи.');
  }
  ...
  function pumpMeetingQueue() {
    while (meetingWorkersRef.current < MEETING_CONCURRENCY && meetingQueueRef.current.length > 0) {
      const item = meetingQueueRef.current.shift();
      meetingWorkersRef.current += 1;
      processMeetingSegment(item.blob, item.index)
        .catch(() => null)
        .finally(() => {
          meetingWorkersRef.current = Math.max(0, meetingWorkersRef.current - 1);
  ...
  function enqueueMeetingSegment(blob, index) {
    ...
    meetingQueueRef.current.push({ blob, index });
    pumpMeetingQueue();
  }

  async function processPreparedMeetingSegment(jobId, preparedIndex, displayIndex, originalName = 'meeting') {
    try {
      const transcription = await transcribePreparedSegment(jobId, preparedIndex);
      setMeetingSegments((items) => [...items.filter((item) => item.index !== displayIndex), { index: displayIndex, text: transcription.text || '' }].sort((a, b) => a.index - b.index));
      setMeetingFailedSegments((items) => items.filter((item) => item.index !== displayIndex));
      return true;
    } catch (error) {
      setMeetingFailedSegments((items) => [
        ...items.filter((item) => item.index !== displayIndex),
        { index: displayIndex, preparedJobId: jobId, preparedIndex, filename: originalName, error: error.message || 'Ошибка расшифровки' },
      ].sort((a, b) => a.index - b.index));
      setGlobalError(`Не удалось расшифровать часть ${displayIndex + 1} после 3 попыток: ${error.message}. Её можно повторить отдельно.`);
      return false;
    }
  }
  ...
  async function retryFailedMeetingSegment(item) {
    ...
    setMeetingPending((value) => value + 1);
    try {
      if (item.preparedJobId) {
        await processPreparedMeetingSegment(item.preparedJobId, item.preparedIndex, item.index, item.filename);
      } else {
  ...
  async function startMeeting() {
    ...
    meetingQueueRef.current = [];
    meetingWorkersRef.current = 0;
    try {
      const stream = await navigator.mediaDevices.getUserMedia({ audio: true });
  ...
  async function uploadMeetingFiles(files) {
    ...
    try {
      for (const file of list) {
        const form = new FormData();
        form.append('audio', file, file.name || 'meeting');
        const prepared = await apiJson('/api/ai/prepare-meeting-audio', { method: 'POST', body: form });
        const count = Number(prepared.segments || 0);
        if (!prepared.jobId || count <= 0) throw new Error('Сервер не смог разделить запись на части.');

        const startIndex = meetingSegmentIndexRef.current;
        meetingSegmentIndexRef.current += count;
        setMeetingPending((value) => value + count);
        let cursor = 0;
        let allOk = true;

        async function worker() {
          while (true) {
            const preparedIndex = cursor;
            cursor += 1;
            if (preparedIndex >= count) return;
            const displayIndex = startIndex + preparedIndex;
            try {
              const ok = await processPreparedMeetingSegment(prepared.jobId, preparedIndex, displayIndex, file.name || 'meeting');
              if (!ok) allOk = false;
            } finally {
              setMeetingPending((value) => Math.max(0, value - 1));
            }
        ...
        const workers = Array.from({ length: Math.min(MEETING_CONCURRENCY, count) }, () => worker());
        await Promise.all(workers);
        if (allOk) {
          apiJson('/api/ai/cleanup-audio-job', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ jobId: prepared.jobId }),
          }).catch(() => null);
        }
      }
    } catch (error) {
      setGlobalError(`Не удалось подготовить запись: ${error.message}`);
    } finally {
      ...
      <div className="grid gap-5 xl:grid-cols-2">
        ...
        <section className="rounded-3xl border bg-white p-5 shadow-sm">
          ...
          <div className="mt-4 flex flex-wrap items-center gap-3">
            ...
            {meetingPreparing && <span className="inline-flex items-center text-sm text-indigo-700"><Loader2 className="mr-2 h-4 w-4 animate-spin" />Подготавливаю запись и делю на части…</span>}{meetingPending > 0 && <span className="inline-flex items-center text-sm text-sky-700"><Loader2 className="mr-2 h-4 w-4 animate-spin" />В очереди / обработке: {meetingPending} · параллельно до {MEETING_CONCURRENCY}</span>}
          </div>
          <label className="mt-4 flex cursor-pointer items-center justify-center rounded-2xl border-2 border-dashed border-slate-200 bg-slate-50 p-4 text-sm text-slate-600 hover:border-sky-300 hover:bg-sky-50"><FileUp className="mr-2 h-5 w-5" />Или загрузить готовую запись РНП целиком (аудио/видео до 500 МБ)<input type="file" accept="audio/*,video/*" multiple disabled={meetingActive || meetingPending > 0 || meetingPreparing} className="hidden" onChange={(event) => { uploadMeetingFiles(event.target.files); event.target.value = ''; }} /></label>
