...
import MavisDragon from './components/MavisDragon';
                      ...
                      <div className="border-t bg-white">
                        {stageTasks.map((task) => (
                          <div key={task.id} className="grid gap-2 border-b px-3 py-3 last:border-b-0 lg:grid-cols-[minmax(240px,2fr)_120px_130px_145px_minmax(170px,1fr)_80px] lg:items-center">
                            ...
                            <span className="flex gap-1"><button type="button" onClick={() => setTaskBacklog(task, true)} className="rounded-lg bg-amber-50 p-2 text-amber-800" title="В бэклог"><Inbox className="h-4 w-4" /></button><button type="button" onClick={() => setTaskArchived(task, true)} className="rounded-lg bg-slate-100 p-2 text-slate-700" title="В архив"><Archive className="h-4 w-4" /></button><button type="button" onClick={() => openTaskModal(task)} className="rounded-lg bg-violet-50 p-2 text-violet-700"><Edit3 className="h-4 w-4" /></button></span>
                          </div>
                        ))}
